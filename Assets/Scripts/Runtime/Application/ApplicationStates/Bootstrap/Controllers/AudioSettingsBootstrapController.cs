...
namespace Runtime.GameStateMachine
{
    public class AudioSettingsBootstrapController : BaseController
    {
        ...
        {
            var isSoundVolume = _userDataService.GetUserData().SettingsData.SoundVolumeValue;
            _audioService.SetVolume(AudioType.Sound, isSoundVolume);

            var isMusicVolume = _userDataService.GetUserData().SettingsData.MusicVolumeValue;
